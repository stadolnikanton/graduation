...
class ShareServices:
    def __init__(
        ...
        file_services: FileUploadService,
    ) -> None:
    ...
    async def get_shared_info(self, token):
        try:
            ...
            if (
                file_info.expires_at is not None
                and file_info.expires_at <= datetime.now()
            ):
                raise TokenTimeOutError()

            if (
                file_info.max_downloads is not None
                and file_info.download_count >= file_info.max_downloads
            ):
                raise TokenMaxDownloadError()
            ...
            return {
                ...
                "expires_at": (
                    file_info.expires_at.isoformat() if file_info.expires_at else None
                ),
